...
async function processMessage(
    ...
    if (!checkForCommand(message) && checkForTrigger(message)) {
        ...
        return botResponse.data.modelResponse;
    }
    console.log('not calling bot');
    ...
    try {
        switch (command) {
            ...
            case `/${commands.bets.command}`: {
                ...
                if (!senderWalletAddress) {
                    return "Sorry, I couldn't find your wallet address.";
                }
            ...
            case `/${commands.copy.command}`: {
                if (!args[0]) {
                    return 'Please specify a market number to copy. Example: /copy 1';
                }
                const senderInboxState = await client.preferences.inboxStateFromInboxIds([conversation.id]);
                const senderWalletAddress = senderInboxState?.[0]?.recoveryIdentifier?.identifier;
                if (!senderWalletAddress) {
...
async function shouldRespondToMessage(
    ...
    const hasTrigger = checkForTrigger(lowerMessage);

    // Check if message contains any command
    const hasCommand = checkForCommand(lowerMessage);

    return hasTrigger || hasCommand;
}
...
const checkForCommand = (message: string) => {
    ...
    return Object.values(commands).some((cmd) => {
        ...
        return lowerMessage.startsWith(`/${cmdStr}`) || lowerMessage.includes(`@onit ${cmdStr}`);
    });
}

const checkForTrigger = (message: string) => {
    const lowerMessage = message.toLowerCase().trim();
    return ONIT_TRIGGERS.some((trigger) =>
        lowerMessage.includes(trigger.toLowerCase()),
    );
}
